...
export default function App() {
  ...
  useEffect(() => {
    const getAllMenu = async () => {
      await axios.get("http://localhost:5000/kahve").then((response) => {
  ...
  const handleChange = (e: any) => {
    const { name, value } = e;
    setKahve({ ...kahve, [name]: value });
  };

  const Ekle = (yeniKahve: any) => {
    axios
      .post("http://localhost:5000/kahve", yeniKahve)
      .then((response) => {
  ...
  const handleKahveSil = async (id: number) => {
    ...
    await axios.delete(`http://localhost:5000/kahve/${id}`).then(() => {
          ...
          <>
            {filteredMenu.map((item: any) => (
              <Card
                ...
                key={item._id}
                isPressable
                onPress={() => router.push(`/pricing/${item._id}`)}
                className="relative"
              >
